...
public class FallbackLanguagesTests
{
    ...
    [Fact]
    public async Task FallbackTranslationTests()
    {
        Assert.Equal("Some Swedish translation", await _sut.GetString("Resource.With.Swedish.Translation", new CultureInfo("sv")));
        Assert.Equal("Some English translation", await _sut.GetString("Resource.With.English.Translation", new CultureInfo("sv")));
        Assert.Equal("Some Norwegian translation", await _sut.GetString("Resource.With.Norwegian.Translation", new CultureInfo("sv")));
        Assert.Equal("Some Norwegian translation", await _sut.GetString("Resource.With.Norwegian.And.English.Translation", new CultureInfo("sv")));
    }

    [Fact]
    public async Task Language_ShouldFollowLanguageBranchSpecs()
    {
        Assert.Equal("Some French translation", await _sut.GetString("Resource.With.FrenchFallback.Translation", new CultureInfo("fr-BE")));
        Assert.Equal("Some English translation", await _sut.GetString("Resource.InFrench.With.EnglishFallback.Translation", new CultureInfo("fr-BE")));

    }

    [Fact]
    public async Task GetStringByNorwegianRegion_ShouldReturnInNorwegian()
    {
        Assert.Equal("Some Latvian translation", await _sut.GetString("Resource.With.Latvian.Translation", new CultureInfo("lv")));
        Assert.Equal("Some Latvian translation", await _sut.GetString("Resource.With.Latvian.Translation", new CultureInfo("lv-LV")));
    }
}
...
public class FallbackLanguagesTestTranslationHandler : IQueryHandler<GetTranslation.Query, string>
{
    ...
    public Task<string> Execute(GetTranslation.Query query)
    {
        return Task.FromResult(
            _resources[query.Key].Translations.GetValueWithFallback(query.Language,
