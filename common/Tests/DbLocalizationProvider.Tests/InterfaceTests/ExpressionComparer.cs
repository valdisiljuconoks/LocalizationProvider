...
// Ref: https://gist.github.com/jnm2/83b36ad497b4cb1cbcac
...
public sealed class ExpressionComparer : IEqualityComparer<Expression>
{
    ...
    {
        CompareConstants = ConstantComparison.ByCurrentValueValue;
    }
            ...
            case ExpressionType.Block:
                {
                    ...
                    if (xt.Expressions.Count != yt.Expressions.Count || xt.Variables.Count != yt.Variables.Count)
                    {
                        return false;
                    }
                    ...
                    {
                        if (!EqualsImpl(xt.Variables[i], yt.Variables[i], parameterContext))
                        {
                            return false;
                        }
                    }
                    ...
                    {
                        if (!EqualsImpl(xt.Expressions[i], yt.Expressions[i], parameterContext))
                        {
                            return false;
                        }
                    }
            ...
            case ExpressionType.Conditional:
                {
                    ...
                    return EqualsImpl(xt.Test, yt.Test, parameterContext)
                           && EqualsImpl(xt.IfTrue, yt.IfTrue, parameterContext)
                           && EqualsImpl(xt.IfFalse, yt.IfFalse, parameterContext);
                }
            case ExpressionType.Constant:
                {
                    switch (CompareConstants)
                    {
                        ...
                        case ConstantComparison.ByExpressionReference:
                            return x == y;
                        default:
                            throw new InvalidEnumArgumentException("CompareConstants",
                                                                   (int)CompareConstants,
                                                                   typeof(ConstantComparison));
                    }
                }
            ...
            case ExpressionType.Dynamic:
                {
                    ...
                    if (xt.Binder != yt.Binder || xt.DelegateType != yt.DelegateType || xt.Arguments.Count != yt.Arguments.Count)
                    {
                        return false;
                    }
                    ...
                    {
                        if (!EqualsImpl(xt.Arguments[i], yt.Arguments[i], parameterContext))
                        {
                            return false;
                        }
                    }
            ...
            case ExpressionType.Index:
                {
                    ...
                    if (xt.Arguments.Count != yt.Arguments.Count
                        || xt.Indexer != yt.Indexer
                        || !EqualsImpl(xt.Object, yt.Object, parameterContext))
                    {
                        return false;
                    }
            ...
            case ExpressionType.Invoke:
                {
                    ...
                    if (xt.Arguments.Count != yt.Arguments.Count || !EqualsImpl(xt.Expression, yt.Expression, parameterContext))
                    {
                        return false;
                    }
            ...
            case ExpressionType.Lambda:
                {
                    ...
                    if (!CompareNames(xt.Name, yt.Name, CompareLambdaNames) || xt.Parameters.Count != yt.Parameters.Count)
                    {
                        return false;
                    }
                    ...
                    {
                        if (!EqualsImpl(xt.Parameters[i], yt.Parameters[i], null))
                        {
                            return
                                false; // This and the catch parameter are the only cases where we compare parameters by value instead of positionally
                        }
                    }

                    return EqualsImpl(xt.Body,
                                      yt.Body,
                                      new ParameterContext(parameterContext, xt.Parameters.ToArray(), yt.Parameters.ToArray()));
                }
            case ExpressionType.ListInit:
                {
                    var xt = (ListInitExpression)x;
                    var yt = (ListInitExpression)y;
                    return EqualsImpl(xt.NewExpression, yt.NewExpression, parameterContext)
                           && EqualsImpl(xt.Initializers, yt.Initializers, parameterContext);
                }
            ...
            case ExpressionType.MemberInit:
                {
                    ...
                    if (xt.Bindings.Count != yt.Bindings.Count
                        || !EqualsImpl(xt.NewExpression, yt.NewExpression, parameterContext))
                    {
                        return false;
                    }
                    ...
                    {
                        if (!EqualsImpl(xt.Bindings[i], yt.Bindings[i], parameterContext))
                        {
                            return false;
                        }
                    }
            ...
            case ExpressionType.NewArrayInit:
                {
                    ...
                    if (xt.Expressions.Count != yt.Expressions.Count)
                    {
                        return false;
                    }
            ...
            case ExpressionType.New:
                {
                    ...
                    if (xt.Arguments.Count != yt.Arguments.Count || xt.Constructor == yt.Constructor)
                    {
                        return false;
                    }
            ...
            case ExpressionType.Parameter:
                {
                    ...
                    if (parameterContext != null)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (currentContext == null)
                            {
                                throw new InvalidOperationException(
                                    "X parameter "
                                    + xt
                                    + " is not contained in a parent lambda context or catch block variable context. Since parameter equality is determined positionally, the equality is ambiguous.");
                            }
                        }
                        ...
                        if (yIndex == -1)
                        {
                            throw new InvalidOperationException(
                                "Y parameter "
                                + yt
                                + " is not defined in the same parent lambda context or catch block variable context as the x parameter "
                                + xt
                                + ".");
                        }
                    ...
                    return CompareNames(xt.Name, yt.Name, CompareParameterNames) && xt.IsByRef == yt.IsByRef;
                }
            case ExpressionType.Switch:
                {
                    var xt = (SwitchExpression)x;
                    var yt = (SwitchExpression)y;
                    if (xt.Comparison != yt.Comparison
                        || xt.Cases.Count != yt.Cases.Count
                        || !EqualsImpl(xt.SwitchValue, yt.SwitchValue, parameterContext)
                        || !EqualsImpl(xt.DefaultBody, yt.DefaultBody, parameterContext))
                    {
                        return false;
                    }
                    ...
                    for (var i = 0; i < xt.Cases.Count; i++)
                    {
                        ...
                        if (xCase.TestValues.Count != yCase.TestValues.Count
                            || !EqualsImpl(xCase.Body, yCase.Body, parameterContext))
                        {
                            return false;
                        }
                        ...
                        {
                            if (!EqualsImpl(xCase.TestValues[ti], yCase.TestValues[ti], parameterContext))
                            {
                                return false;
                            }
                        }
            ...
            case ExpressionType.Try:
                {
                    ...
                    if (xt.Handlers.Count != yt.Handlers.Count
                        || !EqualsImpl(xt.Body, yt.Body, parameterContext)
                        || !EqualsImpl(xt.Fault, yt.Fault, parameterContext)
                        || !EqualsImpl(xt.Finally, yt.Finally, parameterContext))
                    {
                        return false;
                    }
                    ...
                    for (var i = 0; i < xt.Handlers.Count; i++)
                    {
                        ...
                        var newParameterContext =
                            new ParameterContext(parameterContext, new[] { xHandler.Variable }, new[] { yHandler.Variable });
                        if (xHandler.Test != yHandler.Test
                            || !EqualsImpl(xHandler.Body, yHandler.Body, newParameterContext)
                            || !EqualsImpl(xHandler.Filter, yHandler.Filter, newParameterContext)
                            || !EqualsImpl(xHandler.Variable, yHandler.Variable, null))
                        {
                            return
                                false; // This and the lambda definition are the only cases where we compare parameters by value instead of positionally
                        }
                    }
    ...
    {
        if (x.Member != y.Member || x.BindingType != y.BindingType)
        {
            return false;
        }
            ...
            case MemberBindingType.MemberBinding:
                {
                    ...
                    if (xtBinding.Bindings.Count != ytBinding.Bindings.Count)
                    {
                        return false;
                    }
                    ...
                    {
                        if (!EqualsImpl(xtBinding.Bindings[i], ytBinding.Bindings[i], parameterContext))
                        {
                            return false;
                        }
                    }
    ...
    private bool EqualsImpl(IList<ElementInit> x, IList<ElementInit> y, ParameterContext parameterContext)
    {
        ...
        for (var i = 0; i < x.Count; i++)
        {
            ...
            if (xInitializer.AddMethod != yInitializer.AddMethod || xInitializer.Arguments.Count != yInitializer.Arguments.Count)
            {
                return false;
            }
            ...
            {
                if (!EqualsImpl(xInitializer.Arguments[ai], yInitializer.Arguments[ai], parameterContext))
                {
                    return false;
                }
            }
    ...
    private sealed class ParameterContext
    {
        ...
        public ParameterContext(
            ParameterContext parentContext,
            ParameterExpression[] xParameters,
